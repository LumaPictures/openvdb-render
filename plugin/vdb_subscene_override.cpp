...
namespace MHWRender {
    ...
    struct VDBSubSceneOverrideData {
        ...
        bool update(const VDBVisualizerData* data, const MObject& obj)
        {
            ...
            // TODO: handle shader updates separately
            data_has_changed |= setup_parameter(point_size, data->point_size);
            ...
            if (data->display_mode == DISPLAY_POINT_CLOUD) {
                ...
                if (point_jitter > 0.001f) {
                    std::uniform_real_distribution<float> distributionX(
                        -point_jitter * static_cast<float>(voxel_size.x()),
                        point_jitter * static_cast<float>(voxel_size.x()));
                    std::uniform_real_distribution<float> distributionY(
                        -point_jitter * static_cast<float>(voxel_size.y()),
                        point_jitter * static_cast<float>(voxel_size.y()));
                    std::uniform_real_distribution<float> distributionZ(
                        -point_jitter * static_cast<float>(voxel_size.z()),
                        point_jitter * static_cast<float>(voxel_size.z()));
